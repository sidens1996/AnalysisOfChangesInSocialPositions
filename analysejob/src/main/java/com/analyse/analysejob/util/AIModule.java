...
public class AIModule {
	...
	*/
	...
	public void readJobVec() {
		ResultSet rs=dao.execute("select * from jobVectors2");
		try {
			while(rs.next()){
				//System.out.println("enter while");
				// 通过字段检索
    ...
    private void getJobVecMap(Map<Integer,String> map) {
        for(int i:map.keySet()) {
            RealVector vec=str2vec(map.get(i));
            jobVecMap.put(i,vec);
        }
    }
    ...
    //2-27 am
    private String getJobType(List<MyJob> jobs,int n,int m) {
        Map<String,Integer> map=new HashMap();
        for(int i=0;i<n;i++) {
            MyJob job=jobs.get(i);
            String type=getJobTypeById(job.id);
        ...
        return getJobTypeById(jobs.get(0).id);
    }
    public TotalData recommend(String[] tagStr) {
        ...
        for(int i=0;i<10;i++)
            System.out.println(jobs.get(i).getJob_name());
        return s.getTotalData(getJobType(jobs,10,3));
    }
